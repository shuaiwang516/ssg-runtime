 ...
 *
 * <p>The taxonomy is intentionally small — Phase 1 only commits to the
 * dominant upgrade-critical families for each system plus two catch-all
 * buckets ({@link #BACKGROUND}, {@link #UNKNOWN}). Long-tail verb / method
 * coverage is deferred to Phase 5 version-aware profile generation. See
 ...
 *
 * <p>Each family is tagged with a {@link ProtocolFamilyClass} so the scorer
 * can distinguish upgrade-critical traffic from background chatter without
 * re-running the classifier. {@code BACKGROUND} and {@code UNKNOWN} are
 * shared across systems; all other families are system-specific.
 */
public enum ProtocolFamily {
    ...
    /** HBase region-admin lifecycle (open/close/split/merge/compact, WAL replication). */
    ...
    // --- Shared catch-alls ---
    /** Gossip / heartbeat / status-report chatter. Visible but not strong-window-eligible on its own. */
    ...
    /**
     * Stable short token for use in canonical keys. Returns the enum name
     * directly — keys embed it, so renaming an enum constant changes the key
     * and invalidates diff history.
     */
    public String token() {
        return name();
    }
