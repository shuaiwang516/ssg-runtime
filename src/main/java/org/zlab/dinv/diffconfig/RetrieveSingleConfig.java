...
public class RetrieveSingleConfig extends ConfigRetriever implements Runnable {
    ...
    @CommandLine.Option(names = {"-targetSystemPath"}, required = true, description = "path to old system")
    ...
    public static void saveSingleConfigInfo(
            ConfigInfo configInfo, Path outputPath) {
        saveConfigInfo(removeClassInfo(configInfo.classToFieldsWithType), outputPath.resolve("config2Type.json"));
        saveConfigInfo(removeClassInfo(configInfo.classToFieldsWithInit), outputPath.resolve("config2Init.json"));
